            ...
            if (GraphicsSettings.GetRenderPipelineSettings<RenderGraphSettings>().enableRenderCompatibilityMode == false)
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    if (GUILayout.Button(new GUIContent("Enable", EditorGUIUtility.IconContent("d_tab_next").image), GUILayout.Width(60)))
                    {
                        ...
                        EditorUtility.DisplayDialog($"{SCPE.ASSET_NAME} v{SCPE.INSTALLED_VERSION}", "Please note that this option will be removed in a future Unity version, this asset will no longer be functional then.", "OK");
                    }
                    GUILayout.Space(8);
                }
                GUILayout.Space(11);
